...
- Experimental looks: `?pro=1` (AI / dev: sidebar card, animated neural network, typewriter line, gradients, reveal animations) and `?pro=2` (afro design: kente band, bogolan sidebar, earthy palette). `?pro=0` resets. Styles in `src/styles/pro.css`, effects in `src/components/ProEffects.astro`, typewriter phrases in `profile.json` → `pro.terminal` (separated by `|`). Respects `prefers-reduced-motion`.
